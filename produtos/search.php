...
if (isset($_GET['keyword'])) {
    ...
    $sql = "select * from produtos WHERE lower(descricao) LIKE :keyword order by id";
    $sth = $pdo->prepare($sql);
    $sth->bindValue(":keyword", "%" . strtolower($keyword) . "%");
    $sth->execute();
...
<div class="text-center">
    <h4><b>Registro(s) encontrado(s)</b>: <?= count($rows) ?> com <?= $keyword ?></h4>
</div>
